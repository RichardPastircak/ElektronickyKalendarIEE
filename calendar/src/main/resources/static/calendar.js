...
document.getElementById("changePeriod").addEventListener("click", function (){changePeriod()});

document.getElementById("ZmenaA").addEventListener("click", function (){changeShiftName("Zmena A")});
document.getElementById("ZmenaB").addEventListener("click", function (){changeShiftName("Zmena B")});
document.getElementById("ZmenaC").addEventListener("click", function (){changeShiftName("Zmena C")});
document.getElementById("ZmenaD").addEventListener("click", function (){changeShiftName("Zmena D")});
...
const color_data = [
    {
        id: 0,
        name: "blue",
        value: "rgba(225,225,239,0.18)"
    },
    {
        id: 1,
        name: "red",
        value: "rgba(75,58,58,0.87)"
    },
    {
        id: 2,
        name: "purple",
...
let staticCurrentColor = {
    id: 1,
    name: color_data[1].name,
    value: color_data[1].value
};
...
currentYearDOM.style.display = "none";
currentMonthDOM.style.display = "none";
currentDateDOM.style.display = "none";
currentDayDOM.style.display = "none";
        ...
        for (let j = 0; j < 7; j++) {
            ...
            if (
              ...
              `;
                ...
                week.children[j].id = currentWeek[j].day+"/"+(currentFullMonth.month_idx+1);
                week.children[j].classList.add("currMonth");
                // week.children[j].style.backgroundColor = "#e1e1e1";
                week.children[j].style.color = "#b61f2a";
                week.children[j].style.fontSize = "7.5vh";
...
function changeShiftName(shiftName){
    document.getElementById("Zmena").innerText = shiftName;
    showCalenderInfo();
}

function parserData() {

    const currentShift = document.getElementById("Zmena").innerText.replaceAll(" ", "");
    console.log(currentShift);
    const parserData = document.getElementById(currentShift).getAttribute("data-parser").replaceAll(" ","").replaceAll(",","");
    var field = [];
    var tmp = "";

    for (data in parserData){
        if (parserData[data] !== "[" && parserData[data] !== "]" && parserData[data] !== ":"){
            tmp += parserData[data];
        }
        else if (parserData[data] === ":" || parserData[data] === "]"){
            field.push(tmp);
            tmp = ""
        }
    }
    ...
    for (td in tmp) {
        for(item in field) {
            if (field[item] === tmp[td].id) {
                tmp[td].style.backgroundColor = "#"+field[parseInt(item)+1]+"88";
                // console.log("Color for " + tmp[td].id + " is " + field[parseInt(item)+1]);
                // console.log("previous item was " + field[item]);
                break;
            }
        }
    }
...
function changePeriod(){
    ...
    let dayIn=parseInt(day);
    console.log(month);
    console.log(day);
    console.log(year);

    state.todayDay=dayIn-1;
    state.todayMonth=monthIn-1;
...
function getOutputDate(){
    let dateOut= document.getElementById("dateInput2")
    let dateOutValue= dateOut.value;
    dateOutValue.toString();
    console.log(dateOutValue);
    let year=dateOutValue.substr(0,4);
    let month=dateOutValue.substr(5,2);
    let day=dateOutValue.substr(8,2);
    let monthOut=parseInt(month);
    let dayOut=parseInt(day);
    console.log(month);
    console.log(day);
    console.log(year);
}
...
function nextMonth() {
    state.todayMonth += 1;
    if (document.querySelector(".fa-caret-left").style.display === "none"){
        document.querySelector(".fa-caret-left").style.display = "revert";
    }

    if (state.todayMonth == 11) {
...
function prevMonth() {
    state.todayMonth -= 1;
    if (document.querySelector(".fa-caret-right").style.display === "none"){
        document.querySelector(".fa-caret-right").style.display = "revert";
    }
    if (state.todayMonth == 0) {
